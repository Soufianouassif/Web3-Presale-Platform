...
export default function ConnectingPage() {
  ...
  return (
    <div className={`fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-[#0a0a0a] transition-opacity duration-500 ${fadeOut ? "opacity-0" : "opacity-100"}`}>
      <div className="absolute inset-0 bg-gradient-to-br from-[#0a0a0a] via-[#0d1a0d] to-[#0a0a0a]" />
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 max-w-md w-full">
        <div className="relative mb-8">
          {!showSuccess ? (
            <div className="w-24 h-24 sm:w-28 sm:h-28 relative">
              ...
              <div
                className="absolute inset-0 rounded-full border-4 border-transparent border-t-[#00ff88] border-r-[#FF4D9D]"
                style={{ animation: "spin 1s linear infinite" }}
              />
              <div className="absolute inset-2 rounded-full bg-[#0a0a0a] border-2 border-white/10 flex items-center justify-center overflow-hidden">
                <img src="/logo.png" alt="PEPEWIFE" className="w-full h-full object-cover rounded-full" />
              </div>
            </div>
          ) : (
            <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-full bg-[#00ff88] flex items-center justify-center border-4 border-[#00cc66] shadow-[0_0_30px_rgba(76,175,80,0.4)]" style={{ animation: "pulse 1s ease-in-out" }}>
        ...
        {!showSuccess ? (
          <>
            ...
            <div className="sticker bg-[#00ff88] text-black text-xs px-3 py-1 mb-6" style={{ transform: "rotate(-1deg)" }}>
              {t.connecting.securingBag}
            </div>
          </>
        ) : (
          <>
            <h2 className="text-3xl sm:text-4xl font-display text-[#00ff88] tracking-wider comic-shadow mb-2">
        ...
        <div className="w-full meme-card bg-white/5 backdrop-blur rounded-2xl p-5 mb-6 border-white/10">
          <div className="space-y-3 mb-4">
            {steps.map((step, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                {i < currentStep && (
                  <span className="text-xs text-[#00ff88] ms-auto">✓</span>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="flex justify-between mt-1.5">
            ...
            <span className="text-[10px] font-display text-[#ffe000] tracking-wider">{Math.min(Math.round(progress), 100)}%</span>
          </div>
        </div>
        ...
        {showSuccess && (
          <div className="meme-card bg-[#4CAF50]/10 rounded-2xl p-4 w-full mb-4 border-[#4CAF50]/30" style={{ animation: "fadeInUp 0.5s ease-out" }}>
            <div className="flex items-center gap-3">
              <Shield className="w-5 h-5 text-[#00ff88] shrink-0" />
              <div className="text-start">
                <div className="text-xs font-display text-[#00ff88] tracking-wider">{t.connecting.walletConnected}</div>
